...
public class AP_Manager : MonoBehaviour {
	...
	public bool allowCreate = true;// 将允许使用MF_AutoPool.InitializePool（）的脚本创建新的对象池。
    public bool allowModify = true;// 将允许使用MF_AutoPool.InitializePool（）的脚本修改现有的对象池。

    [Tooltip("退出播放器时，打印控制台中所有池的使用情况摘要以进行开发和调试：\n\n" +
        "Start Size - 开始场景时池的大小。\n\n" +
        "Init Added - 运行时InitializeSpawn（）调用添加的对象数。\n\n" +
        "Grow Objects - 通过EmptyBehaviour.Grow添加的对象数。\n\n" +
        "End Size - 日志报告时，此池的活动和非活动对象总数。\n\n" +
        "Failed Spawns - 未返回衍生的Spawn（）请求的数量。\n\n" +
        "Reused Objects - 对象在正常派生之前被重用的次数。\n\n" +
        "Most Objects Active - 一次最多活动此池的所有项目。")]
